...
export function BusinessOutcomesPanel() {
  ...
  useEffect(() => {
    const onRegisterUpdated = (event: Event) => {
      const register = (event as CustomEvent<LeadRegister>).detail;
      if (!register) return;
      setData(summarizeLeadRegisterOutcomes(register));
      setError(false);
    };
    window.addEventListener("lead-register:updated", onRegisterUpdated);
    return () => window.removeEventListener("lead-register:updated", onRegisterUpdated);
  }, []);
  return <section className="mb-6 rounded-xl border p-5" aria-label="HighLevel contact outcomes">
    ...
    {data && <><div className="mt-4 grid grid-cols-2 gap-4 md:grid-cols-3 xl:grid-cols-6">{[["People with form receipts",data.peopleEnquiring],["Form-attributed, receipt missing",data.formOriginWithoutReceipt],["Tagged contacted",data.contactedNewContacts],["Booked people",data.uniqueBookers],["Booked appointments",data.appointments],["No shows",data.noShows]].map(([label,value]) => <div key={label}><p className="text-sm text-muted-foreground">{label}</p><p className="text-2xl font-semibold">{value}</p></div>)}</div>
    <p className="mt-4 text-sm">{data.formSubmissions} form submission receipts from {data.peopleEnquiring} people. {data.formOriginWithoutReceipt} additional contacts have configured-form attribution but no submission receipt in HighLevel’s feed. {data.contactsCreated} new CRM contacts were created in the same window.</p>
